...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        {
            FitToHud,      // Keep aspect ratio, fit inside HUD
            FillHud,       // Fill HUD completely (may crop)
            OriginalSize   // Use original window size
        }
        
        private WindowCaptureScaleMode _windowCaptureScaleMode = WindowCaptureScaleMode.FitToHud;
        ...
        public void Activate()
        {
            ...
            // Restore window capture settings if available
            RestoreWindowCapture();
        ...
        private void setWindowCaptureToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (_isWindowCaptureActive)
            {
                // Stop window capture
                StopWindowCapture();
                setWindowCaptureToolStripMenuItem.Text = "Capture Window";
                windowCaptureOptionsToolStripMenuItem.Visible = false;
                Settings.Instance.Remove("HUD_WindowCaptureHandle");
                Settings.Instance.Remove("HUD_WindowCaptureTitle");
            }
            else
            {
        ...
        private void scaleMode_Click(object sender, EventArgs e)
        {
            var clickedItem = (ToolStripMenuItem)sender;
            
            // Uncheck all scale mode items
            fitToHudToolStripMenuItem.Checked = false;
            fillHudToolStripMenuItem.Checked = false;
            originalSizeToolStripMenuItem.Checked = false;
            
            // Check the clicked item and set mode
            clickedItem.Checked = true;
            
            if (clickedItem == fitToHudToolStripMenuItem)
            {
                _windowCaptureScaleMode = WindowCaptureScaleMode.FitToHud;
                Settings.Instance["HUD_WindowCaptureScaleMode"] = "FitToHud";
            }
            else if (clickedItem == fillHudToolStripMenuItem)
            {
                _windowCaptureScaleMode = WindowCaptureScaleMode.FillHud;
                Settings.Instance["HUD_WindowCaptureScaleMode"] = "FillHud";
            }
            else if (clickedItem == originalSizeToolStripMenuItem)
            {
                _windowCaptureScaleMode = WindowCaptureScaleMode.OriginalSize;
                Settings.Instance["HUD_WindowCaptureScaleMode"] = "OriginalSize";
            }
            
            log.Info($"Window capture scale mode changed to: {_windowCaptureScaleMode}");
        }

        private void ShowWindowSelectionDialog()
        {
            try
            {
                using (var windowSelectionForm = new WindowSelectionForm())
                {
            ...
            {
                MessageBox.Show($"Error showing window selection dialog: {ex.Message}", "Error", 
                               MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void RestoreWindowCapture()
        {
            try
            {
                // Restore scale mode setting
                if (Settings.Instance.ContainsKey("HUD_WindowCaptureScaleMode"))
                {
                    string scaleMode = Settings.Instance["HUD_WindowCaptureScaleMode"];
                    if (Enum.TryParse<WindowCaptureScaleMode>(scaleMode, out var mode))
                    {
                        _windowCaptureScaleMode = mode;
                        
                        // Update UI
                        fitToHudToolStripMenuItem.Checked = (mode == WindowCaptureScaleMode.FitToHud);
                        fillHudToolStripMenuItem.Checked = (mode == WindowCaptureScaleMode.FillHud);
                        originalSizeToolStripMenuItem.Checked = (mode == WindowCaptureScaleMode.OriginalSize);
                    }
                }
                
                if (Settings.Instance.ContainsKey("HUD_WindowCaptureHandle") && 
                    Settings.Instance.ContainsKey("HUD_WindowCaptureTitle"))
                {
                    string handleStr = Settings.Instance["HUD_WindowCaptureHandle"];
                    if (long.TryParse(handleStr, out long handleLong))
                    {
                        IntPtr handle = new IntPtr(handleLong);
                        string title = Settings.Instance["HUD_WindowCaptureTitle"];
                        
                        // Check if window still exists
                        var windows = WindowCapture.GetVisibleWindows();
                        var existingWindow = windows.FirstOrDefault(w => w.Handle == handle || w.Title == title);
                        
                        if (existingWindow.Handle != IntPtr.Zero)
                        {
                            StartWindowCapture(existingWindow.Handle);
                            log.Info($"Restored window capture for: {existingWindow.Title}");
                        }
                        else
                        {
                            log.Info($"Previously captured window '{title}' no longer available");
            ...
            {
                log.Error("Error restoring window capture", ex);
                // Clear invalid settings
                Settings.Instance.Remove("HUD_WindowCaptureHandle");
                Settings.Instance.Remove("HUD_WindowCaptureTitle");
            }
        ...
        private void StartWindowCapture(IntPtr windowHandle)
        {
            ...
            {
                MessageBox.Show($"Error starting window capture: {ex.Message}", "Error", 
                               MessageBoxButtons.OK, MessageBoxIcon.Error);
                log.Error("Error starting window capture", ex);
            }
        }
        ...
        private void StopWindowCapture()
        {
            try
            {
                if (_windowCapture != null)
                {
                    _windowCapture.FrameCaptured -= OnFrameCaptured;
                    _windowCapture.StopCapture();
                ...
                log.Info("Window capture stopped");
            }
            catch (Exception ex)
            {
                log.Error("Error stopping window capture", ex);
            }
        }
        ...
        private void UpdateHudBackground(Bitmap frame)
        {
            try
            {
                var oldImage = hud1.bgimage;
                
                if (_windowCaptureScaleMode == WindowCaptureScaleMode.OriginalSize)
                {
                    // Use original size without scaling
                    hud1.bgimage = new Bitmap(frame);
                }
                else if (hud1.Width <= 0 || hud1.Height <= 0)
                {
                    // HUD size not available, use original
                    hud1.bgimage = new Bitmap(frame);
                }
                else
                {
                    // Scale to HUD size with selected mode
                    var scaledImage = new Bitmap(hud1.Width, hud1.Height);
                    using (var g = Graphics.FromImage(scaledImage))
                    {
                        ...
                        float scaleX = (float)hud1.Width / frame.Width;
                        float scaleY = (float)hud1.Height / frame.Height;
                        float scale;
                        
                        if (_windowCaptureScaleMode == WindowCaptureScaleMode.FitToHud)
                        {
                            // Fit inside HUD, maintain aspect ratio (black bars if needed)
                            scale = Math.Min(scaleX, scaleY);
                        }
                        else // FillHud
                        {
                            // Fill HUD completely, maintain aspect ratio (may crop)
                        ...
                        // Center the image
                        int x = (hud1.Width - newWidth) / 2;
                        int y = (hud1.Height - newHeight) / 2;
                        
                        g.DrawImage(frame, x, y, newWidth, newHeight);
                    }
                    
                    hud1.bgimage = scaledImage;
                }
            ...
            catch (Exception ex)
            {
                ...
                hud1.bgimage = new Bitmap(frame);
                oldImage?.Dispose();
                log.Error("Error scaling HUD background, using original size", ex);
            }
        }
